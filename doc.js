...
function navPath(path) {
  ...
  if (currentPage) { currentPage.$a.removeClass('open'); }
...
function initPager() {
  ...
  .fail(function(jqXHR) { console.log('unable to load /pages.json'); })
  .done(function(data) {
...
function initSearchNav() {
  ...
  $('#toc a').each(function(){
    var $a = $(this);
    var o = { href:     $a.attr('href').slice(pubRef.relPath.length),
              $a:       $a,
              text:     $a.text(),
  ...
  $(window).keydown(function(evt) {
    ...
    if (k === 37) { navTo(currentPage && currentPage.prev) } // left
    if (k === 39) { navTo(currentPage && currentPage.next) } // right
    return true;
  })
